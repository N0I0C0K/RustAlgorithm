    ...
    T: Default + Sized + Copy + Add<Output = T> + Mul<Output = T> + Eq + AddAssign + From<usize>,
{
    ...
    pub fn query(&mut self, l: usize, r: usize) -> T {
        self.range_query(l, r, 0, self.n - 1, 1)
    }

    pub fn add(&mut self, l: usize, r: usize, val: T) {
        self.range_add(l, r, 0, self.n - 1, 1, val)
    }
}
...
mod test {
    ...
    fn range_add() {
        ...
        assert_eq!(seg.query(0, 8), 36);
        seg.add(0, 8, 10);
        assert_eq!(seg.query(1, 8), 116);
        assert_eq!(seg.query(0, 8), 126);
    }
}
